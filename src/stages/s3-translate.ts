...
export function lengthRule(fillToTarget: boolean): string {
  if (fillToTarget) {
    return [
      ...
      '   Если реплика получается длиннее — режь второстепенное.',
      '   Если короче — **скажи то же самое иначе**, а не дописывай. У мысли почти всегда есть',
      '   более полное русское выражение, занимающее время честно: «Ты убил троих наших» →',
      '   «Из-за тебя погибли трое наших»; «Мира больше нет» → «О мире больше не может быть речи».',
      '   Дописывание слышно сразу и звучит фальшиво: «Лиза.» → «Лиза, Лиза», «Ты убил троих',
      '   наших людей, помнишь?» — это не укладка, а вата. Не нашлось естественной формулировки',
      '   нужной длины — оставь короче, это меньшее зло.',
    ].join('\n');
  }
  return [
