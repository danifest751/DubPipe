...
const MESSAGES: Record<string, Record<UiLanguage, string>> = {
  ...
  'ready.ollama.title': { ru: 'Локальный перевод — модель {model}', en: 'Local translation — {model} model' },
  'ready.ollama.noDaemon': { ru: 'Ollama не отвечает на {endpoint}', en: 'Ollama is not answering at {endpoint}' },
  'ready.ollama.noModel': { ru: 'модель не скачана; установлено: {installed}', en: 'the model is not pulled; installed: {installed}' },
  'ready.ollama.none': { ru: 'моделей не установлено', en: 'no models installed' },
  'ready.ollama.blocks': {
    ru: 'стадия S3: переводить будет нечем',
    en: 'stage S3: there will be nothing to translate with',
  },
  'ready.ollama.startHint': { ru: 'Запустите Ollama: ollama serve', en: 'Start Ollama: ollama serve' },
  'ready.ollama.pullHint': { ru: 'Скачайте модель: ollama pull {model}', en: 'Pull the model: ollama pull {model}' },
  'ready.size.ollama': { ru: 'модель качается отдельно', en: 'the model is downloaded separately' },
