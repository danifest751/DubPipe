 ...
 *
 * Пути собираются через `path.join`: тесты идут и на Linux, где обратный слэш —
 * обычный символ, и литеральный `C:\…` там не разбирается на части. На этом CI
 * уже падал.
 */
...
const cachedSource = (extension: string) => path.join('cache', `source.${extension}`);

describe('§FR-7: откуда S7 берёт источник', () => {
  it('для ссылки — путь, записанный S1: input там URL', () => {
    const downloaded = path.join('videos', 'Ролик [dQw4w9WgXcQ].mp4');
  ...
  it('находит источник в старом рабочем каталоге', () => {
    ...
    const cached = path.join('cache', 'source.mp4');
    const source = pickSource({ input: 'https://youtu.be/dQw4w9WgXcQ' }, cachedSource, existing(cached));
    expect(source).toBe(cached);
  });

  it('для локального файла берёт сам вход', () => {
    const file = path.join('videos', 'lecture.mp4');
    const source = pickSource({ input: file }, cachedSource, existing(file));
    expect(source).toBe(path.resolve(file));
  });

  it('приоритет у записанного пути, даже если в кэше что-то осталось', () => {
    const downloaded = path.join('videos', 'новое.mp4');
    const stale = path.join('cache', 'source.mp4');
    const source = pickSource(
  ...
  it('если скачанное удалили, ошибка называет путь, где его искали', () => {
    const downloaded = path.join('videos', 'Ролик [id].mp4');
    const attempt = () => pickSource({ input: 'https://youtu.be/id', source_path: downloaded }, cachedSource, existing());

    expect(attempt).toThrow(StageError);
    try {
      attempt();
    } catch (error) {
  ...
  it('для ссылки без записанного пути подсказывает перезапуск S1', () => {
    try {
      pickSource({ input: 'https://youtu.be/id' }, cachedSource, existing());
